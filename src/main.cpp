/*An estimation model of human dynamic arm strength with joint rotation compensation*/
...
int main() {
	...
	for (int m = 0; m < filedata_array.size(); m++) {
		data_block.row(m) = Eigen::VectorXd::Map(filedata_array[m].data(), filedata_array[m].size());
	}
